...
export default function makePlayersMongoDb(makeDb: () => Promise<Db>): IplayerDb {
  ...
  async function insertObject(name: string, obj: IPlayer): Promise<InsertOneWriteOpResult<any>> {
    //update the player's data
    const db = await makeDb()
    const updateResult = await db.collection('players').updateOne({ name }, { $set: obj })
